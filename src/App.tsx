...
import { Smartphone, ExternalLink, Star, MessageCircle, ChevronDown, Check, Globe } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen text-primary selection:bg-accent-blue/30 overflow-x-hidden">
      <div className="bg-cinematic">
        ...
        <div className="glow-blob w-[500px] h-[500px] bg-accent-cyan/5 bottom-[-100px] right-[-100px]" style={{ animationDelay: '-5s' }} />
      </div>
    ...
    <nav className="sticky top-0 z-50 liquid-glass border-x-0 border-t-0 border-b border-white/10 py-4 px-6 mb-8">
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-8 text-sm font-medium text-secondary">
          <a href="#how" className="hover:text-primary transition">{t.nav.works}</a>
          <a href="#products" className="hover:text-primary transition">{t.nav.products}</a>
          <a href="#businesses" className="hover:text-primary transition">{t.nav.businesses}</a>
          <a href="#faq" className="hover:text-primary transition">{t.nav.faq}</a>
        </div>

        <div className="flex items-center gap-4">
          <button onClick={toggleLang} className="text-sm font-medium opacity-70 hover:opacity-100 flex items-center gap-1">
    ...
    <section className="pt-20 md:pt-32 pb-10 flex flex-col md:flex-row items-center gap-16">
      ...
      >
        ...
        <div className="flex flex-col gap-3 mt-4">
          <a 
            href={buildWhatsAppUrl(t.whatsapp.general)} target="_blank" rel="noreferrer"
          ...
          <span className="text-sm text-secondary text-center">{t.hero.subCta}</span>
        </div>
      </motion.div>
      ...
      >
        ...
        >
          ...
          <div className="text-xs font-medium text-accent-cyan uppercase tracking-widest">NFC Active</div>
        </motion.div>
      </motion.div>
    </section>
  );
  ...
  return (
    <section id="how" className="space-y-12 scroll-m-24">
      <div className="text-center space-y-4">
        <h2 className="text-xs font-bold tracking-widest text-secondary uppercase">{t.works.eyebrow}</h2>
        <h3 className="text-4xl md:text-5xl font-black">{t.works.title1} <span className="text-secondary">{t.works.title2}</span></h3>
      </div>
        ...
        {t.works.steps.map((step: any, idx: number) => {
          ...
          return (
            <motion.div 
              ...
              whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 30 }} viewport={{ once: true, amount: 0.2 }}
            >
...
function Products({ t }: { t: any }) {
  return (
    <section id="products" className="space-y-16 scroll-m-24">
      <div className="text-center space-y-4">
        <h2 className="text-4xl md:text-5xl font-black">{t.products.title1} <span className="text-secondary">{t.products.title2}</span></h2>
      ...
      <div className="grid md:grid-cols-2 gap-8">
        ...
        >
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-3xl font-black mb-2">{t.products.googleTitle}</h3>
              <p className="text-secondary">{t.products.googleDesc}</p>
            </div>
            <ul className="space-y-3 font-medium text-sm text-secondary">
              {['NFC technology', 'Direct Google review link', 'Personalized design', 'No app required', 'No subscription'].map((f: string, i: number) => (
                <li key={i} className="flex items-center gap-2"><Check className="w-4 h-4 text-accent-cyan" /> {f}</li>
              ))}
            </ul>
          </div>
          <div className="mt-12 flex items-center justify-between border-t border-white/10 pt-6">
            ...
            <a 
              href={buildWhatsAppUrl(t.whatsapp.google)} target="_blank" rel="noreferrer"
        ...
        >
          ...
          <div className="space-y-6 relative z-10">
            ...
            <div>
              <h3 className="text-3xl font-black mb-2">{t.products.instagramTitle}</h3>
              <p className="text-secondary">{t.products.instagramDesc}</p>
            </div>
            <ul className="space-y-3 font-medium text-sm text-secondary">
              {['NFC technology', 'Direct Instagram profile link', 'Personalized design', 'No app required', 'No subscription'].map((f: string, i: number) => (
                <li key={i} className="flex items-center gap-2"><Check className="w-4 h-4 text-fuchsia-400" /> {f}</li>
              ))}
            </ul>
          </div>
          <div className="mt-12 flex items-center justify-between border-t border-white/10 pt-6 relative z-10">
            ...
            <a 
              href={buildWhatsAppUrl(t.whatsapp.instagram)} target="_blank" rel="noreferrer"
              className="bg-white text-black px-6 py-3 rounded-full font-bold hover:bg-gray-200 transition"
            >
              {t.nav.order}
            </a>
  ...
  return (
    <div className="grid md:grid-cols-2 gap-4 text-center text-sm p-6 liquid-glass rounded-3xl">
      <div className="border-b md:border-b-0 md:border-e border-white/10 pb-4 md:pb-0 md:pe-4">
        ...
        <p className="text-secondary">Best for: Restaurants, cafés, clinics, local services</p>
      </div>
      <div className="pt-4 md:pt-0 md:ps-4">
        <h4 className="font-bold text-lg mb-2">Instagram</h4>
        <p className="text-secondary">Best for: Fashion, beauty, retail, creators</p>
      </div>
    </div>
  );
}
...
function Benefits({ t }: { t: any }) {
  return (
    <section className="space-y-12">
      <div className="text-center space-y-4">
        <h2 className="text-xs font-bold tracking-widest text-secondary uppercase">{t.benefits.eyebrow}</h2>
        <h3 className="text-4xl md:text-5xl font-black">{t.benefits.title1} <span className="text-secondary">{t.benefits.title2}</span></h3>
      </div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {t.benefits.items.map((item: any, idx: number) => (
          <div key={idx} className="liquid-glass p-6 rounded-2xl flex flex-col gap-2 border-t border-t-white/10">
            <div className="text-accent-cyan font-black text-xl">0{idx + 1}</div>
...
function BusinessTypes({ t }: { t: any }) {
  return (
    <section id="businesses" className="space-y-8 scroll-m-24 overflow-hidden">
      <div className="text-center space-y-4">
        <h2 className="text-xs font-bold tracking-widest text-secondary uppercase">{t.business.eyebrow}</h2>
        <h3 className="text-3xl md:text-4xl font-bold max-w-2xl mx-auto">{t.business.title}</h3>
      </div>
      <div className="flex overflow-x-auto gap-4 pb-4 no-scrollbar -mx-6 px-6 md:mx-0 md:px-0 md:flex-wrap md:justify-center">
        {t.business.types.map((type: string, i: number) => (
          <div key={i} className="liquid-glass px-6 py-3 rounded-full text-sm font-medium whitespace-nowrap border border-white/10">
            {type}
          </div>
        ))}
      </div>
      ...
      <div className="liquid-glass rounded-[2rem] p-8 md:p-16 max-w-3xl w-full flex flex-col items-center gap-10">
        <div className="flex bg-black/50 p-1 rounded-full border border-white/10">
          <button 
            ...
            className={`px-6 py-2 rounded-full text-sm font-bold transition ${demoType === 'google' ? 'bg-white text-black' : 'text-secondary hover:text-white'}`}
          >
            Google
          </button>
          <button 
            onClick={() => setDemoType('instagram')}
            className={`px-6 py-2 rounded-full text-sm font-bold transition ${demoType === 'instagram' ? 'bg-white text-black' : 'text-secondary hover:text-white'}`}
          >
            Instagram
          </button>
        </div>
        ...
        <div className="relative w-full h-[300px] flex items-center justify-center gap-8 md:gap-20">
          <motion.div className="liquid-glass w-24 h-24 md:w-32 md:h-32 rounded-xl flex items-center justify-center border-t border-white/20 shadow-lg">
            {demoType === 'google' ? <Star className="text-white opacity-80" /> : <Smartphone className="text-white opacity-80" />}
          </motion.div>
          
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex gap-2 opacity-50">
            {[1, 2, 3].map((i: number) => (
              <motion.div key={i} className="w-1 h-8 bg-accent-cyan rounded-full" 
                animate={{ opacity: [0.2, 1, 0.2] }} transition={{ repeat: Infinity, duration: 1.5, delay: i * 0.2 }}
              />
            ))}
          </div>
              ...
              >
                {demoType === 'google' ? (
                  <div className="text-center space-y-3">
                    ...
                    <div className="text-[10px] text-gray-400">Share your experience</div>
                    <div className="flex gap-1 justify-center text-yellow-500"><Star size={12}/><Star size={12}/><Star size={12}/><Star size={12}/><Star size={12}/></div>
                  </div>
                ) : (
                  <div className="text-center space-y-3">
                    ...
                    <div className="bg-accent-blue text-white text-[10px] py-1 px-4 rounded font-bold">Follow</div>
                  </div>
                )}
              </motion.div>
            </AnimatePresence>
  ...
  return (
    <footer className="border-t border-white/10 py-12 px-6">
      <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-6 text-sm text-secondary">
        <div className="flex items-center gap-4">
          <div className="font-bold text-xl text-primary tracking-tight">tapi tapi</div>
